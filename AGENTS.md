...
- **Domain:** `nutri_points`
- **Title:** Nutri Points
- **Repository:** megageek/nutripoints-homeassistant

**Key directories:**

- `custom_components/nutri_points/` - Main integration code
...
- Integration logs: `custom_components.nutri_points: debug` in `config/configuration.yaml`
...
- Use the domain `nutri_points` for all DOMAIN references
- Prefix all integration-specific classes with `NutriPoints`
- Use "Nutri Points" as the display title
...
- Inherit from platform base + `NutriPointsEntity`
...
- `tests/` mirrors `custom_components/nutri_points/` structure
