...
async def _async_render_service_value(hass: HomeAssistant, value: Any, field: str) -> Any:
    ...
    try:
        ...
        raise ServiceValidationError(f"Invalid template for '{field}': {exc}") from exc
...
async def _async_parse_int(
    ...
    *,
    ...
    try:
        ...
        raise ServiceValidationError(f"'{field}' must resolve to an integer.") from exc
    if parsed < minimum:
        raise ServiceValidationError(f"'{field}' must be >= {minimum}.")
    return parsed
...
async def _async_parse_string(
    ...
    *,
    ...
    try:
        ...
        raise ServiceValidationError(f"'{field}' must resolve to text.") from exc
    if len(parsed) < min_length:
        raise ServiceValidationError(f"'{field}' must be at least {min_length} character(s).")
    if max_length is not None and len(parsed) > max_length:
        raise ServiceValidationError(f"'{field}' must be {max_length} characters or fewer.")
    return parsed
...
async def _async_parse_optional_date(hass: HomeAssistant, value: Any, field: str) -> date | None:
    ...
    try:
        ...
        raise ServiceValidationError(f"'{field}' must resolve to a date (YYYY-MM-DD).") from exc
...
async def _async_parse_optional_datetime(hass: HomeAssistant, value: Any, field: str) -> datetime | None:
    ...
    try:
        ...
        raise ServiceValidationError(f"'{field}' must resolve to an ISO datetime value.") from exc
...
async def _async_parse_optional_meal_type(hass: HomeAssistant, value: Any) -> str | None:
    ...
    if parsed not in MEAL_TYPES:
        ...
        raise ServiceValidationError(f"'{SERVICE_ATTR_MEAL_TYPE}' must be one of: {allowed}.")
    return parsed
...
async def _async_resolve_food_nutrition(hass: HomeAssistant, call: ServiceCall) -> dict[str, float | int | str]:
    ...
    if nutrition_entry_mode == "points":
        if SERVICE_ATTR_POINTS not in call.data:
            raise ServiceValidationError(f"'{SERVICE_ATTR_POINTS}' is required when nutrition_entry_mode is 'points'.")
    ...
    for field in (SERVICE_ATTR_PROTEIN_G, SERVICE_ATTR_CARBS_G, SERVICE_ATTR_FAT_G, SERVICE_ATTR_FIBER_G):
        if field not in call.data:
            raise ServiceValidationError(f"'{field}' is required when nutrition_entry_mode is 'macros'.")
...
async def _async_parse_non_zero_int(
    ...
    *,
    ...
    if parsed == 0:
        raise ServiceValidationError(f"'{field}' must not be 0.")
    if maximum_abs is not None and abs(parsed) > maximum_abs:
        raise ServiceValidationError(f"'{field}' magnitude must be <= {maximum_abs}.")
    return parsed
...
async def _async_resolve_drink_type_id(
    ...
    *,
    ...
    if not isinstance(drink_types, list):
        raise ServiceValidationError("Nutri Points returned invalid drink settings data.")
    ...
    if not matches:
        raise ServiceValidationError(f"Drink type '{drink_type_name}' was not found.")
    if len(matches) > 1:
        raise ServiceValidationError(
    ...
    if not isinstance(resolved_id, int) or resolved_id < 1:
        raise ServiceValidationError("Resolved drink type id is invalid.")
    return resolved_id
